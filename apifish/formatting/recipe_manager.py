...
def check_recipe(recipe, data_directory=None):
    ...
    """
    # check parameters
    utils.check_parameter(recipe=dict, data_directory=(str, type(None)))
    ...
    if not isinstance(recipe_pattern, str):
        raise TypeError(
            "'pattern' should be a string, not a {0}.".format(type(recipe_pattern))
        )
...
def fit_recipe(recipe):
    ...
    """
    # check parameters
    utils.check_parameter(recipe=dict)
...
def get_path_from_recipe(recipe, input_folder, fov=0, r=0, c=0, z=0):
    ...
    """
    # check parameters
    utils.check_parameter(recipe=dict, input_folder=str, fov=int, r=int, c=int, z=int)
...
def count_nb_fov(recipe):
    ...
    if not isinstance(recipe["fov"], list):
        raise TypeError(
            "'fov' should be a List or a str, not {0}".format(type(recipe["fov"]))
        )
    else:
...
def check_datamap(data_map):
    ...
    """
    utils.check_parameter(data_map=list)
    for pair in data_map:
        ...
        if len(pair) != 2:
            raise ValueError(
                ...
                "(string). Here {0} elements are given {1}".format(len(pair), pair)
            )
