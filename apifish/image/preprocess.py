...
def compute_image_standardization(image):
    ...
    if len(image.shape) != 2:
        raise ValueError(
            "'image' should be a 2-d array. Not {0}-d array".format(len(image.shape))
        )
    ...
    for r in range(tensor.shape[0]):
        ...
        for c in range(tensor.shape[1]):
            ...
            if c in channel_to_stretch:
                ...
                channel_rescaled = rescale_intensity(channel, out_range=target_range)
...
def cast_img_uint8(tensor):
    ...
    if (
        tensor.dtype in [np.uint16, np.uint32, np.uint64, np.int16, np.int32, np.int64]
        and tensor.max() <= 255
    ):
        raise ValueError(
            "Tensor values are between {0} and {1}. It fits in 8 "
            "bits and won't be scaled between 0 and 255. Use "
            "'tensor.astype(np.uint8)' instead.".format(tensor.min(), tensor.max())
        )
...
def cast_img_uint16(tensor):
    ...
    if (
        ...
        raise ValueError(
            ...
            "Use 'tensor.astype(np.uint16)' instead.".format(tensor.min(), tensor.max())
        )
...
def unmix_channels(image, number_channels=2):
    ...
    output_images = [
        np.zeros((number_z_planes, im_size[1], im_size[2]))
